...
def decode_bias(loc, priors, variances):
    """Decode locations from predictions using priors to undo
    the encoding we did for offset regression at train time.
    Args:
        loc (tensor): location predictions for loc layers,
    ...
    Return:
        decoded bounding box predictions ([x_min, y_min, x_max, y_max])
    """
    center_bias_xy = loc[:, :2] * variances[0] * priors[:, 2:]
    wh_bias = torch.exp(loc[:, 2:] * variances[1])
    center_bias = (center_bias_xy[:, 0]*center_bias_xy[:, 0] + center_bias_xy[:, 1]*center_bias_xy[:, 1]).sqrt()
...
def cos_similarity(roi_features, tubelets, device='cuda'):
    similarity = torch.zeros(len(roi_features), len(tubelets)).to(device)
